...
# Title:  Week 4 Programming Assignment 3 - worst.R function
...
worst <- function(state, outcome) {
        ...
        if(state %in% data[, 7] & outcome %in% c("heart attack", 
                ...
                if(outcome %in% c("heart attack")) {
                        ...
                        hsp <- st_data[which.max(st_data[, 11]), 2]
                }
                else if(outcome %in% c("heart failure")) {
                        st_data <- data[data[, 7] %in% state, ]
                        st_data <- st_data[order(st_data[, 2]), ]
                        hsp <- st_data[which.max(st_data[, 17]), 2]
                }
                else {
                        st_data <- data[data[, 7] %in% state, ]
                        st_data <- st_data[order(st_data[, 2]), ]
                        hsp <- st_data[which.max(st_data[, 23]), 2]
                }
        }
